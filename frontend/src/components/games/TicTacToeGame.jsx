...
const ROWS = 15;
const COLS = 15;
const START_R = 6, END_R = 8;
const START_C = 6, END_C = 8;
...
const TicTacToeGame = forwardRef(({ onWinnerChange, onCursorChange }, ref) => {
  ...
  const checkWin = (b) => {
    const lines = [
      ...b, // Hàng ngang
      [b[0][0], b[1][0], b[2][0]], [b[0][1], b[1][1], b[2][1]], [b[0][2], b[1][2], b[2][2]], // Dọc
      [b[0][0], b[1][1], b[2][2]], [b[0][2], b[1][1], b[2][0]] // Chéo
    ];
  ...
  const renderButton = (r, c) => {
    ...
    const isInPlayArea = r >= START_R && r <= END_R && c >= START_C && c <= END_C;
    ...
    if (isInPlayArea) {
      ...
      colorClass = val === "X" ? "bg-green-500" : val === "O" ? "bg-red-500" : "bg-gray-700";
      if (val) content = <span className="text-white font-bold text-xs">{val}</span>;
    }

    return (
      <div key={`${r}-${c}`} className={`w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center transition-all ${colorClass} ${isCursor ? "ring-4 ring-white scale-110 z-10 opacity-100" : ""}`}>
        {content || (isCursor && <div className="w-2 h-2 bg-white rounded-full animate-pulse" />)}
      </div>
    );
  };
