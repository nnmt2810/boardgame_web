...
const SnakeGame = forwardRef(({ onWinnerChange, onCursorChange }, ref) => {
  ...
  const reportScore = useCallback(async (finalScore) => {
  ...
  const generateFood = useCallback((currentSnake) => {
    ...
    while (true) {
      ...
      if (!currentSnake.some(seg => seg[0] === newFood[0] && seg[1] === newFood[1])) break;
    }
    setFood(newFood);
  }, []);
  ...
  // ... (Phần renderButton và return giữ nguyên)
  const renderButton = (r, c) => {
    const isHead = snake[0][0] === r && snake[0][1] === c;
    const isBody = snake.some((seg, i) => i !== 0 && seg[0] === r && seg[1] === c);
    const isFood = food[0] === r && food[1] === c;
    let colorClass = isHead ? "bg-green-400 z-10 scale-110" : isBody ? "bg-green-700 opacity-80" : isFood ? "bg-red-500 animate-pulse" : "bg-gray-800";
    return <div key={`${r}-${c}`} className={`w-6 h-6 sm:w-8 sm:h-8 rounded-sm ${colorClass}`} />;
  };

  return (
    <div className="bg-black p-4 rounded-3xl border-12 border-gray-800 shadow-2xl">
      <div className="grid gap-1" style={{ gridTemplateColumns: `repeat(${COLS}, minmax(0, 1fr))` }}>
        {Array.from({ length: ROWS }).map((_, r) => Array.from({ length: COLS }).map((_, c) => renderButton(r, c)))}
      </div>
